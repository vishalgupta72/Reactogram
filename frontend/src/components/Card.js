...
const Card = (props) => {
    ...
    //       }
    //     } catch (error) {
    //       console.error("Error liking the post:", error.response?.data?.error || error.message);
    //       // Optionally, show an alert to the user if they try to like a post multiple times
    ...
    const likeDislikePost = async (postId, type) => {
        ...
        try {
          ...
          console.error(`Error ${type === 'like' ? 'liking' : 'unliking'} the post:`, 
            error.response?.data?.error || error.message);
      
          // Optionally, show an alert to the user
          alert(error.response?.data?.error || `An error occurred while trying to ${type === 'like' ? 'like' : 'unlike'} the post.`);
        }
      };
          ...
          <div className="row">
            <div className="col-12">
              <img
                ...
                src={props.postData?.image?.url || props.postData?.image}
                alt="post"
              />
            </div>
          </div>
          ...
          <div className="row mt-2">
            <div className="col-6 d-flex">
              <i
                ...
                className="fa-regular fa-heart fs-5 pe-2 ps-2"
              ></i>
              <i
                onClick={() => likeDislikePost(props.postData._id, "unlike")}
                className="fa-regular fa-thumbs-down fs-5 pe-2 ps-2"
              ></i>
              <i
                onClick={() => setCommentBox(true)}
                className="fa-regular fa-comment fs-5 pe-2"
              ></i>
              <i className="fa-solid fa-location-arrow fs-5 pe-2"></i>
            </div>

            <div className="col-6">
              <span className="pe-2 fw-bold fs-6 float-end">
